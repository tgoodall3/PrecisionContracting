...
import { CONTACT_EMAIL, CONTACT_PHONE, SERVICE_AREAS, TAGLINE } from '@/config';
    ...
    <Container className="grid gap-10 py-12 md:grid-cols-4">
      ...
      <div>
        ...
        <div className="mt-6 space-y-1 text-sm text-brand-navy/80">
          <p>Email: <a href={`mailto:${CONTACT_EMAIL}`} className="text-brand-blue hover:underline">{CONTACT_EMAIL}</a></p>
          <p>Phone: <a href={`tel:${CONTACT_PHONE.replace(/\D/g, '')}`} className="text-brand-blue hover:underline">{CONTACT_PHONE}</a></p>
        </div>
      </div>
    </Container>
